...
import 'package:sharehandscustomer/AppGlobal.dart';
import 'package:sharehandscustomer/constants.dart';
import 'package:sharehandscustomer/model/OrderModel.dart';
...
import 'package:sharehandscustomer/model/VendorModel.dart';
import 'package:sharehandscustomer/model/variant_info.dart';
...
import 'package:sharehandscustomer/ui/chat_screen/chat_screen.dart';
import 'package:sharehandscustomer/ui/orderDetailsScreen/order_tracking_screen.dart';
import 'package:sharehandscustomer/ui/reviewScreen.dart/reviewScreen.dart';
...
class OrderDetailsScreen extends StatefulWidget {
  ...
  const OrderDetailsScreen({Key? key, this.orderModel, this.orderId}) : super(key: key);
...
class _OrderDetailsScreenState extends State<OrderDetailsScreen> {
  ...
  loadData() async {
    if (widget.orderModel != null) {
      ...
      await FireStoreUtils().getSectionsById(orderModel!.sectionId).then((value) {
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        sectionConstantModel!.serviceTypeFlag != "ecommerce-service" &&
                            ...
                            ? orderModel.estimatedTimeToPrepare!.isEmpty
                                ? SizedBox()
                                : Padding(
              ...
              orderModel!.takeAway == false
                  ? Column(
                      ...
                      children: [
                        ...
                        Text(
                          'Address'.tr(),
                          style: TextStyle(fontSize: 16, letterSpacing: 0.5, color: isDarkMode(context) ? Colors.grey.shade200 : AppThemeData.primary300),
                        ),
            ...
            children: [
              ...
              ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    VariantInfo? variantIno = orderModel.products[index].variant_info;
                    ...
                    return FutureBuilder<ProductModel>(
                      future: FireStoreUtils().getProductByID(orderModel.products[index].id),
                      builder: (context, snapshot) {
                            ...
                            Row(
                              ...
                              children: [
                                CachedNetworkImage(
                                    ...
                                    width: 55,
                                    // width: 50,
                                    imageUrl: getImageVAlidUrl(orderModel.products[index].photo),
                                ...
                                Visibility(
                                    visible: snapshot.data!.isDigitalProduct == true && orderModel.status == ORDER_STATUS_COMPLETED ? true : false,
                                    child: InkWell(
                                      onTap: () async {
                                ...
                                Expanded(
                                  child: InkWell(
                                    ...
                                    onTap: () async {
                                      ...
                                      await FireStoreUtils().getProductByID(orderModel.products[index].id).then((value) {
                                        productModel = value;
                                      });
                                      if (productModel!.itemAttributes != null) {
                                        if (productModel!.itemAttributes!.variants!.where((element) => element.variant_sku == variantIno!.variant_sku).isNotEmpty) {
                                          if (int.parse(productModel!.itemAttributes!.variants!
                                                  .where((element) => element.variant_sku == variantIno!.variant_sku)
                                                  .first
                                                  .variant_quantity
                                                  .toString()) >=
                                        ...
                                        if (productModel!.quantity >= orderModel.products[index].quantity || productModel!.quantity == -1) {
                                          final bool _productIsInList = cartProducts.any((product) =>
                                              product.id == productModel!.id + "~" + (productModel!.variant_info != null ? productModel!.variant_info!.variant_id.toString() : ""));
                                          if (_productIsInList) {
                                            CartProduct element = cartProducts.firstWhere((product) =>
                                                product.id ==
                                                productModel!.id + "~" + (productModel!.variant_info != null ? productModel!.variant_info!.variant_id.toString() : ""));
              ...
              orderModel.takeAway == false
                  ? ListTile(
                      ...
                      trailing: Text(
                        orderModel.deliveryCharge == null ? amountShow(amount: "0") : amountShow(amount: orderModel.deliveryCharge!),
                        style: TextStyle(
                          letterSpacing: 0.5,
                          fontSize: 16,
                          color: isDarkMode(context) ? Colors.grey.shade300 : const Color(0xff333333),
              ...
              orderModel.takeAway == false
                  ? ListTile(
                      ...
                      trailing: Text(
                        orderModel.tipValue!.isEmpty ? amountShow(amount: "0.0") : amountShow(amount: orderModel.tipValue),
                        style: TextStyle(
                          letterSpacing: 0.5,
                          fontSize: 16,
                          color: isDarkMode(context) ? Colors.grey.shade300 : const Color(0xff333333),
              ...
              (orderModel.notes != null && orderModel.notes!.isNotEmpty)
                  ? ListTile(
                      ...
                      trailing: InkWell(
                        ...
                        child: Text(
                          "View".tr(),
                          style: TextStyle(fontSize: 18, color: AppThemeData.primary300, letterSpacing: 0.5),
                        ),
                      ),
                    )
                  : Container(),
                ...
                child: Padding(
                  ...
                  child: InkWell(
                    child: Container(
                        padding: const EdgeInsets.only(top: 8, bottom: 8),
                        decoration:
                            BoxDecoration(color: AppThemeData.primary300, borderRadius: BorderRadius.circular(5), border: Border.all(width: 0.8, color: AppThemeData.primary300)),
                        child: Center(
                          child: Text(
  ...
  Future<List<int>> getTicket() async {
    ...
    bytes += generator.row([
      PosColumn(
          text: orderModel!.takeAway == false ? 'Deliver to door'.tr() : 'Takeaway'.tr(),
          width: 12,
          styles: const PosStyles(align: PosAlign.right, height: PosTextSize.size1, width: PosTextSize.size1, bold: true)),
    ]);
    bytes += generator.row([
    ...
    bytes += generator.row([
      PosColumn(
          text: DateFormat('dd-MM-yyyy, HH:mm').format(DateTime.fromMicrosecondsSinceEpoch(orderModel!.createdAt.microsecondsSinceEpoch)).toString(),
          width: 12,
          styles: const PosStyles(align: PosAlign.right, height: PosTextSize.size1, width: PosTextSize.size1, bold: true)),
    ]);
    bytes += generator.hr();
...
//   ]);
    ...
    bytes += generator.row([
      ...
      PosColumn(
          text: orderModel!.specialDiscount != null ? orderModel!.specialDiscount!['special_discount'].toDouble().toStringAsFixed(currencyData!.decimal) : '0',
          width: 3,
          styles: const PosStyles(
            align: PosAlign.right,
            height: PosTextSize.size1,
    ...
    bytes += generator.row([
      ...
      PosColumn(
          text: orderModel!.deliveryCharge == null ? "0.0" : double.parse(orderModel!.deliveryCharge.toString().replaceAll(',', '').replaceAll('\€', '')).toString(),
    ...
    if (orderModel!.taxModel != null) {
      for (var element in orderModel!.taxModel!) {
        taxAmount = (double.parse(taxAmount) + getTaxValue(amount: (total - discount).toString(), taxModel: element)).toString();
      }
    }

    var totalamount = orderModel!.deliveryCharge == null || orderModel!.deliveryCharge!.isEmpty
        ? total + double.parse(taxAmount) - discount
        : total + double.parse(taxAmount) + double.parse(orderModel!.deliveryCharge!) + tipValue - discount;
                      ...
                      RichText(
                        text: TextSpan(children: [
                          TextSpan(
                              ...
                              style: TextStyle(fontFamily: AppThemeData.semiBold, color: isDarkMode(context) ? Colors.grey.shade200 : Colors.grey.shade600, fontSize: 17)),
                          TextSpan(
                            text: '\n${order.driver?.carNumber ?? 'No car number provided'.tr()}',
                            style: TextStyle(fontFamily: AppThemeData.semiBold, fontSize: 18, color: isDarkMode(context) ? Colors.grey.shade200 : Colors.grey.shade800),
                          ),
                        ]),
                      ),
                      Stack(
  ...
  getPriceTotalText(CartProduct s) {
    ...
    return Text(
      amountShow(amount: total.toString()),
      style: TextStyle(fontSize: 20, color: AppThemeData.primary300),
    );
  }
