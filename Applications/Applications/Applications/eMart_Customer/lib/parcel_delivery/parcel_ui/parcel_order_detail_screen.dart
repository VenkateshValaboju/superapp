...
import 'package:sharehandscustomer/parcel_delivery/parcel_dashboard.dart';
import 'package:sharehandscustomer/parcel_delivery/parcel_model/parcel_order_model.dart';
import 'package:sharehandscustomer/parcel_delivery/parcel_ui/history_screen.dart';
...
class ParcelOrderDetailScreen extends StatefulWidget {
  ...
  const ParcelOrderDetailScreen({Key? key, this.orderModel, this.orderId}) : super(key: key);
      ...
      if (orderModel!.taxModel != null) {
        for (var element in orderModel!.taxModel!) {
          taxAmount = (double.parse(taxAmount) +
                  getTaxValue(amount: (double.parse(orderModel!.subTotal.toString()) - double.parse(orderModel!.discount.toString())).toString(), taxModel: element))
              .toString();
        }
      }
      print("ORDERID");
      print(orderModel!.id);
      totalAmount = amountShow(amount: (double.parse(orderModel!.subTotal!.toString()) - double.parse(orderModel!.discount!.toString()) + double.parse(taxAmount)).toString());
    }
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        leading: GestureDetector(
          onTap: () {
            push(context, ParcelDahBoard(user: MyAppState.currentUser, drawerSelection: DrawerSelection.Orders, appBarTitle: "History".tr(), currentWidget: const HistoryScreen()));
          },
          child: Icon(
                        ...
                        children: [
                          ...
                          Row(
                            ...
                            children: [
                              ...
                              //  buildOtherDetails(title: "Rate".tr(), value: symbol + double.parse(orderModel!.subTotal!).toStringAsFixed(decimal), color: AppThemeData.primary300),
                              buildOtherDetails(title: "Rate".tr(), value: amountShow(amount: orderModel!.subTotal!.toString()), color: AppThemeData.primary300),
                            ],
                          ),
                          const Divider(),
                                    ...
                                    children: [
                                      ClipRRect(
                                        ...
                                        child: CachedNetworkImage(
                                          ...
                                          errorWidget: (context, url, error) => Image.network(placeholderImage, fit: BoxFit.fill),
                                        ),
                                      ),
                                      const SizedBox(
                                        width: 10,
                                        ...
                                        child: Column(
                                          ...
                                          children: [
                                            Text(
                                              ...
                                              style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w600,fontFamily: AppThemeData.regular,),
                                            ),
                                            const SizedBox(
                                              height: 2,
                                            ),
                                            Text(
                                              "Your shipper".tr(),
                                              style: TextStyle(color: Colors.black.withOpacity(0.60),fontFamily: AppThemeData.regular,),
                                            ),
                                          ],
                                        ),
                                      )
    ...
    return GestureDetector(
      child: Padding(
        ...
        child: Column(
          mainAxisAlignment: MainAxisAlignment.spaceEvenly,
          children: [
            Text(title, style: const TextStyle(color: Colors.grey,fontFamily: AppThemeData.regular,)),
            const SizedBox(
              height: 5,
            ),
            Text(value, textAlign: TextAlign.center, style: TextStyle(fontSize: 14,fontFamily: AppThemeData.regular,)),
          ],
        ),
      ),
    );
      ...
      children: [
        ...
        Row(
          ...
          children: [
            Text(
              "Subtotal".tr(),
              style: TextStyle(color: isDarkMode(context) ? const Color(0xffFFFFFF) : const Color(0xff888888), fontSize: 16,fontFamily: AppThemeData.regular,),
            ),
            Text(
        ...
        Row(
          ...
          children: [
            Text(
              "Discount".tr(),
              style: TextStyle(color: isDarkMode(context) ? const Color(0xffFFFFFF) : const Color(0xff888888), fontSize: 16,fontFamily: AppThemeData.medium,),
            ),
            Text(
                ...
                Row(
                  children: [
                    ...
                    Text(
                      amountShow(
                          amount:
                              getTaxValue(amount: (double.parse(orderModel!.subTotal.toString()) - double.parse(orderModel!.discount.toString())).toString(), taxModel: taxModel)
                                  .toString()),
                      style: TextStyle(
                        fontSize: 16,
                        fontFamily: AppThemeData.semiBold,
        ...
        Row(
          ...
          children: [
            ...
            Text(
              ...
              amountShow(
                  amount: ((double.parse(orderModel!.subTotal!.toString())) - double.parse(orderModel!.discount!.toString()) + double.parse(taxAmount.toString())).toString()),
              style: TextStyle(
                fontSize: 18,
                fontFamily: AppThemeData.semiBold,
                color: AppThemeData.primary300,
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            userDetails.phone!,
            style: TextStyle(fontSize: 16, color: Colors.grey.shade500,fontFamily: AppThemeData.medium,),
          ),
          const SizedBox(
            height: 5,
          ),
          Text(
            userDetails.address!,
            maxLines: 3,
            style: TextStyle(color: Colors.grey.shade500,fontFamily: AppThemeData.medium,),
          ),
        ],
      ),
    );
