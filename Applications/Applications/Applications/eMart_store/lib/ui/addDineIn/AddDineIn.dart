...
class _AddDineInState extends State<AddDineIn> {
  ...
  void initState() {
    ...
    FireStoreUtils.getVendor(MyAppState.currentUser!.vendorID)?.then((value) {
      if (value != null) {
        vendors = value;
        dineInFor2price.text = vendors!.vendorCost.toString();
        ...
        _mediaFiles.addAll(vendors!.vendorMenuPhotos);
    ...
    return Scaffold(
        ...
        body: (MyAppState.currentUser!.vendorID.isEmpty)
            ? Container(
                ...
                child: showEmptyState('', 'Please add a store first'.tr()),
              )
                            ...
                            Container(
                              ...
                              child: TextFormField(
                                  ...
                                  decoration: InputDecoration(
                                    ...
                                    prefix: Text('${currencyData!.symbol}'),
  ...
  validate() async {
    if (MyAppState.currentUser!.vendorID != '') {
      if (_formKey.currentState?.validate() ?? false) {
         ShowToastDialog.showLoader('Updating Restaurant...'.tr());
        List menuPhotos = [];
        ...
        vendors!.vendorCost = int.parse(dineInFor2price.text);
        vendors!.enabledDiveInFuture = isDineActive;
        vendors!.openDineTime = time1.text;
        vendors!.closeDineTime = time2.text;
        vendors!.vendorMenuPhotos = menuPhotos;

        await FireStoreUtils.updateVendor(vendors!).then((value) async {
          print("----->Done");
