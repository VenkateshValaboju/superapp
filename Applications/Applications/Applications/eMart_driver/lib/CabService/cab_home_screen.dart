...
import 'package:sharehandsdriver/CabService/verify_otp_screen.dart';
...
import 'package:sharehandsdriver/services/send_notification.dart';
import 'package:sharehandsdriver/theme/app_them_data.dart';
import 'package:sharehandsdriver/ui/chat_screen/chat_screen.dart';
...
class _CabHomeScreenState extends State<CabHomeScreen> with SingleTickerProviderStateMixin {
  ...
  getData() async {
    ...
    driverStream.listen((event) {
      ...
      if (_driverModel!.ordercabRequestData == null) {
        if(selectedMapType == "osm"){
          if (mounted && mapOsmController.initPosition != null) {
            setState(() {
              osmMarkers.clear();
  ...
  getOrders() async {
    while (true) {
      ...
      orders.forEach((element) {
        ...
        print('---->${orderModel.id}');
        orderModel.trigger_delevery = Timestamp.now();
        print("--66-->${double.parse(orderModel.distance!).toStringAsFixed(2)}");
        orderModel.distance=double.parse(orderModel.distance!).toStringAsFixed(2);
        FirebaseFirestore.instance.collection(RIDESORDER).doc(element.id).set(orderModel.toJson(), SetOptions(merge: true)).then((order) {
          print('Done.');
        });
      });
      ...
      driverStream.listen((event) {
        ...
        if (_driverModel!.isActive) {
          ...
          if (orders.isNotEmpty && _driverModel!.inProgressOrderID == null) {
            for (var o in orders) {
              if (o.rejectedByDrivers != null) {
                if (!o.rejectedByDrivers!.contains(_driverModel!.userID)) {
                  _driverModel!.ordercabRequestData = o;
                  break;
                }
              } else {
                _driverModel!.ordercabRequestData = o;
